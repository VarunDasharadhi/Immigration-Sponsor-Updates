...
import { checkSponsorStatus, fetchSponsorNews } from '../services/geminiService';
...
export const SponsorChecker: React.FC = () => {
  ...
  useEffect(() => {
    const loadNews = async () => {
        const updates = await fetchSponsorNews();
        setNews(updates);
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
        const data = await checkSponsorStatus(searchTerm);
        setResult(data);
    } catch (err) {
        // Fallback result to ensure the UI shows something even if API fails
        setResult({
            ...
            dateGranted: 'Unknown',
            sponsorType: 'Unknown',
            notes: 'Unable to verify at this time. System might be busy.',
            history: []
        });
    } finally {
        setLoading(false);
             ...
             <div className="relative z-10">
                ...
                <div className="space-y-6">
                    {newsLoading ? (
                        ...
                        news.map((item, idx) => {
                             ...
                             return (
                                <div key={idx} className="group border-b border-slate-700 pb-4 last:border-0 last:pb-0">
                                    ...
                                    <h4 className="font-bold text-slate-100 mb-1 leading-snug group-hover:text-indigo-300 transition-colors">
                                        {item.title}
                                    </h4>
                        ...
                        <p className="text-slate-400 text-sm">No recent register changes found.</p>
                    )}
                </div>
                ...
                <div className="mt-8 pt-6 border-t border-slate-700">
                    ...
                    >
                        Download Full CSV Register <ArrowRight className="w-3 h-3" />
                    </a>
                </div>
             </div>
        </div>
